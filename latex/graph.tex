...
\node[largenode] (placeholder)    at ({-180/\N*6-180:\r}) {};

\node[largenode, below=of parameters] (gui) {GUI};
...
\path[simuarrow] (print)      edge[bend left=10] (placeholder);


\path[simuarrow] (placeholder)  edge[dashed, bend left] node[below, sloped] {} (gui);
\path[simuarrow] (gui)  edge[dashed, bend left] node[above=0.5, sloped] {Advance Simulation} (game);
%\path[simuarrow, dashed] (placeholder)  edge[bend right] node[below, sloped] {End simulation} ($ (placeholder) + (5, -3) $);
...
\path[inputarrow] (wall)     edge[bend left=65] (placeholder);
\path[inputarrow] (agent)    edge[bend left=55] (placeholder);
\path[inputarrow] (constant) edge[bend left=50] (placeholder);
\path[inputarrow] (goal)     edge[bend left=50] (placeholder);
\path[inputarrow] (result)   edge[bend left=60] (placeholder);
...
\node[inactive, below=1.0 of strategy] (clock) {Poisson clock};
...
\node[largenode, below left=1.0 of gui] (animation) {Animation};
\node[largenode, below right=1.0 of gui] (pygt) {PyQt4};
\node[largenode, right=of pygt] (pyqtgraph) {pyqtgraph};

\path[inputarrow] (gui) edge[] (animation);
\path[inputarrow] (gui) edge[] (pygt);
\path[inputarrow] (pyqtgraph) edge[] (pygt);
